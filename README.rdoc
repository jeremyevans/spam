...
A demo site is available at http://spam-demo.herokuapp.com, with
...
This app can be easily cloned and run on heroku.  You just need to make
sure you are using the heroku-postgresql:dev database (PostgreSQL 9.1), not
the old shared database (PostgreSQL 8.3).  Mainly this is because the
triggers used by the application require plpgsql.

  git clone git://github.com/jeremyevans/spam.git
  cd spam
  heroku create
  heroku config:set STDOUT_LOGGER=1 SECRET_TOKEN="your secret token"
...
Running this outside of Heroku is fairly easy:
  ...
  git clone git://github.com/jeremyevans/spam.git
  cd spam
  cat "your secret token" > config/secret_token.txt
  bundle install
  createdb -U postgres spam
  psql -U postgres spam < config/schema.sql

Manually setup your user:

  psql -U postgres spam
  # INSERT INTO users (name, password_hash) VALUES ('name', 'bcrypt hash');
...
from ActiveRecord to Sequel after taking over maintenance of Sequel. Still,
much of the code is pre-Rails 1 style with fat controllers.

There is a unit and an integration test suite, but both are pretty minimal.
